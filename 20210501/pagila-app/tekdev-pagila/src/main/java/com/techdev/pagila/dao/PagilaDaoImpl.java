...
public class PagilaDaoImpl implements PagilaDao {
	...
	private static final String UPDATE_ACTOR = "UPDATE pagila.actor SET "
			+ "first_name = '%s', last_name= '%s' WHERE actor_id = %s";
	private static final String DELETE_ACTOR = "DELETE from pagila.actor where actor_id = %s";
	...
	public Long updateActor(Actor actor) {
		...
		String query = formatQuery(UPDATE_ACTOR, actor.getFirstname(),
	...
	public Long deleteActor(Actor actor) {
		...
		String query = formatQuery(DELETE_ACTOR, actor.getActorid());
		return executeUpdate(query);
